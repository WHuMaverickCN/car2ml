...
def ST_build_index(vehicle_data_items, index_path='spatial_index'):
    ...
    #     return idx_dict
        ...
        # with open(index_path, 'rb') as f:
        #     idx_dict_pre = pickle.load(f)

        # def serialize_index(index):
        #     items = index.intersection(index.bounds)
        #     return pickle.dumps(sorted(items))
        
        # def indexes_are_equal(index1, index2):
        #     return serialize_index(index1) == serialize_index(index2)
        
        # equal = indexes_are_equal(idx_dict[0], \
        #                           idx_dict_pre[0])
